...
import logging # available: logging.* : debug(''), info, warning, error, critical
...
if not args.host:
    args.host = input('FTP server address (default \'{}\'): '.format(host_default))
    if not args.host:
...
#with pysftp.Connection(host=myhost, username=myuser, password=mypass) as sftp:
#    print(sftp.listdir())
#    print(sftp.pwd)
#    sftp.close()
